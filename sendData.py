...
#        urlContext = []
#        for i in range(0, len(self.URLIDData)):
#            URLID = self.URLIDData[i][0]
...
class Untitled(unittest.TestCase):
    ...
    def test_untitled(self):
        ...
        driver.find_element_by_id("id").send_keys("muerte")
        driver.find_element_by_id("pwd").clear()
        driver.find_element_by_id("pwd").send_keys("caijun*!~")
        ...
        sql = URLID.sqlURLID()
        conn = sql.connectData()
        print(URLIDDataWebdriver)
        for i in range(0, len(URLIDDataWebdriver)):
            URLID = URLIDDataWebdriver[i][0]
            parseURLWeb = parseURL(str(URLID))
            if parseURLWeb.parseURL():
                print('\n')
                titleName = str(sql.searchNameTitleFromSqlite3(conn,URLID)[0]).replace('(','').replace(')','').replace("'","").replace(',',' ')
                print(titleName)
...
#        driver.get(self.base_url + "/nForum/article/Test/post")
#        driver.find_element_by_id("post_subject").clear()
#        driver.find_element_by_id("post_subject").send_keys("test222eeeewwwwTest")
#        driver.find_element_by_id("post_content").click()
#        driver.find_element_by_id("post_content").clear()
#        driver.find_element_by_id("post_content").send_keys("223232")
#        driver.find_element_by_css_selector("input.button").click()
